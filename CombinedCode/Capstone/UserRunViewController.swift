...
class UserRunViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    @IBAction func StartRunTime(sender: UIButton) {
        ...
        time = NSTimer.scheduledTimerWithTimeInterval(1, target: self, selector: #selector(UserRunViewController.RunTimer), userInfo: nil, repeats: true)
        refreshDistance =  NSTimer.scheduledTimerWithTimeInterval(1, target: self, selector: #selector(UserRunViewController.updateData), userInfo: nil, repeats: true)
